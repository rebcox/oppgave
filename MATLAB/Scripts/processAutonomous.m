...
magnitude = 0.15;
plotDataWithMotion(dod_auto,channel, motions, s_auto, magnitude, false, 'DOD without bandpass filter');

%% Plot concentration without bandpass filter
[hbo, hbr, hbt] = convertToConcentrations(dod_auto, SD, ppf);
%[s_auto, hbo_auto] = findAutonomousPart(s,hbo);
%magnitude = max(max(dod_auto));
magnitude = 0.00004;
plotDataWithMotion(hbo, channel, motions, s_auto, magnitude, false, 'Concentration without bandpass filter');
...
magnitude = 0.15;
plotDataWithMotion(dod_auto_filt, channel, motions, s_auto, magnitude, false, 'OD after bandpass filter');

%% Plot concentration with bandpass filter
[hbo_filt, hbr_filt, hbt_filt] = convertToConcentrations(dod_auto_filt, SD, ppf);
%[s_auto, hbo_auto_filt] = findAutonomousPart(s,hbo_filt);
%magnitude = max(max(dod_auto));
magnitude = 0.00003;
plotDataWithMotion(hbo_filt, channel, motions, s_auto, magnitude, false, 'Concentration after bandpass filter');
